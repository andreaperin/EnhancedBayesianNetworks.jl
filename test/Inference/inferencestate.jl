@testset "Inference State" begin
    ...
    @test_throws ErrorException("all nodes in the assignment have to be in the network") InferenceState(bn, :V, Dict(:H => :yesH))

    @test_throws ErrorException("node states must be coherent with the one defined in the network") InferenceState(bn, :V, Dict(:S => :yesH))
