@testset "Evaluation Node" begin
    ...
    root3 = DiscreteNode(:C, DataFrame(:C => [:c1, :c2], :Prob => [0.5, 0.5]), Dict(:c1 => [Parameter(1, :C)], :c2 => [Parameter(2, :C)]))
    ...
    @testset "Continuous Node" begin
        ...
        @test typeof.(evaluated.cpt[!, :Prob]) == [EmpiricalDistribution, EmpiricalDistribution, EmpiricalDistribution, EmpiricalDistribution]
        ...
        @test typeof.(evaluated.cpt[!, :Prob]) == [EmpiricalDistribution]
        ...
        @testset "Imprecise Parents" begin
            ### ROOT
            r1 = DiscreteNode(:A, DataFrame(:A => [:a1, :a2], :Prob => [0.5, 0.5]), Dict(:a1 => [Parameter(1, :A)], :a2 => [Parameter(2, :A)]))
            r2 = ContinuousNode{Tuple{<:Real,<:Real}}(:B, DataFrame(:Prob => (0.1, 0.3)))
            r3 = ContinuousNode{UnivariateDistribution}(:P, DataFrame(:Prob => Uniform(-10, 10)))
    ...
    @testset "Discrete Node" begin
        ...
        @test isapprox(evaluated.cpt[!, :Prob], [0.16, 0.84, 0.5, 0.5]; atol=0.02)
        ...
        @test isapprox(evaluated.cpt[!, :Prob], [0.1587, 0.8413]; atol=0.1)
        @test evaluated.parameters == disc_functional.parameters
        ...
        @testset "Imprecise Parents" begin
            ...
            @test evaluated.cpt[!, :A] == [:a1, :a1, :a2, :a2]
            @test evaluated.cpt[!, :C] == [:C_fail, :C_safe, :C_fail, :C_safe]
            @test isapprox(evaluated.cpt[!, :Prob][1], [0.45191, 0.51868]; atol=0.2)
            @test isapprox(evaluated.cpt[!, :Prob][2], [0.48132, 0.54809]; atol=0.2)
            @test isapprox(evaluated.cpt[!, :Prob][3], [0.50418, 0.56792]; atol=0.2)
            @test isapprox(evaluated.cpt[!, :Prob][4], [0.43208, 0.49582]; atol=0.2)
            ...
            root1 = DiscreteNode(:A, DataFrame(:A => [:a1, :a2], :Prob => [0.5, 0.5]), Dict(:a1 => [Parameter(1, :A)], :a2 => [Parameter(2, :A)]))
            root2 = ContinuousNode{UnivariateDistribution}(:B, DataFrame(:Prob => Normal()))
            root3 = DiscreteNode(:D, DataFrame(:D => [:d1, :d2], :Prob => [0.5, 0.5]), Dict(:d1 => [Parameter(1, :D)], :d2 => [Parameter(2, :D)]))
            states = DataFrame(:A => [:a1, :a2], :Prob => [(0.1, 0.3), (0.7, 0.8)])
            ...
            @test isapprox(evaluated.cpt[!, :Prob][1], [0.198, 0.2406]; atol=0.2)
            @test isapprox(evaluated.cpt[!, :Prob][2], [0.759, 0.802]; atol=0.2)
            @test isapprox(evaluated.cpt[!, :Prob][3], [0.371, 0.4248]; atol=0.2)
            @test isapprox(evaluated.cpt[!, :Prob][4], [0.5752, 0.629]; atol=0.2)
            @test isapprox(evaluated.cpt[!, :Prob][5], [0.537, 0.6224]; atol=0.2)
            @test isapprox(evaluated.cpt[!, :Prob][6], [0.377, 0.463]; atol=0.2)
            @test isapprox(evaluated.cpt[!, :Prob][7], [0.749, 0.7869]; atol=0.2)
            @test isapprox(evaluated.cpt[!, :Prob][8], [0.213, 0.251]; atol=0.2)
        end
    end
end