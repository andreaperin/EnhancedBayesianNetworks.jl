...
function concentrationplt2dict(file_path::String, var_regex::Regex, day_regex::Regex, x_regex::Regex, z_regex::Regex, quantity_name::Symbol)
    ...
    z_values = []
    for (key, val) in parsed_dict
        if length(parsed_dict[key]) == 3 * x * z && isempty(x_values) && isempty(z_values)
    ...
    for (key, val) in parsed_dict
        if length(parsed_dict[key]) == 3 * x * z
            ...
            result["day=$key"][!, quantity_name] = parsed_dict[key][3:3:end]
            result["day=$key"][!, :x] = x_values
            result["day=$key"][!, :z] = z_values
        elseif length(parsed_dict[key]) == x * z
            result["day=$key"] = DataFrame()
            result["day=$key"][!, quantity_name] = parsed_dict[key]
            result["day=$key"][!, :x] = x_values
            result["day=$key"][!, :z] = z_values
        end
    end
...
function tempertatureplt2dict(file_path::String, var_regex::Regex, day_regex::Regex, x_regex::Regex, z_regex::Regex, quantity_name::Symbol)
    data = readdlm(file_path, '\t', skipstart=0)
    result = Dict()
    parsed_dict = Dict()
    day = []
    ...
    ### Checking matrix dimension
    for (key, val) in parsed_dict
        if length(parsed_dict[key]) != x * z * 2 && length(parsed_dict[key]) != 4 * x * z
            println("error parsing values")
        end
    end
    t = 0
    ...
    z_values = []
    for (key, val) in parsed_dict
        if length(parsed_dict[key]) == 4 * x * z && isempty(x_values) && isempty(z_values)
            append!(x_values, parsed_dict[key][1:4:end])
            append!(z_values, parsed_dict[key][2:4:end])
        end
    end
    ## Result dictionary
    for (key, val) in parsed_dict
        if length(parsed_dict[key]) == 4 * x * z
            result["day=$key"] = DataFrame()
            result["day=$key"][!, quantity_name] = parsed_dict[key][3:4:end]
            result["day=$key"][!, :x] = x_values
            result["day=$key"][!, :z] = z_values
        elseif length(parsed_dict[key]) == 2 * x * z
            result["day=$key"] = DataFrame()
            result["day=$key"][!, quantity_name] = parsed_dict[key][1:2:end]
            result["day=$key"][!, :x] = x_values
            result["day=$key"][!, :z] = z_values
        end
    end
...
function headplt2dict(file_path::String, var_regex::Regex, day_regex::Regex, x_regex::Regex, z_regex::Regex, quantity_name::Symbol)
    data = readdlm(file_path, '\t', skipstart=0)
    result = Dict()
    parsed_dict = Dict()
    day = []
